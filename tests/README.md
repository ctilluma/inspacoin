...
Core tests take longer than any other InSPa tests, due to the high amount of computational work involved in validating core components.
...
To run only InSPa's core tests (after building):
...
To run only InSPa's crypto tests (after building):
...
```
inspacoind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `inspacoin-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only InSPa's hash tests (after building):
...
To run only InSPa's performance tests (after building):
...
To run only InSPa's unit tests (after building):
